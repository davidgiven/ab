...
$(OBJ)/tests/pkg+fallbacklib/pkg+fallbacklib.a $(OBJ)/tests/pkg+fallbacklib/fallback.h &:
	...
	$(hide) cp tests/pkg/fallback.h $(OBJ)/tests/pkg+fallbacklib/tests/pkg/fallback.h
...
$(OBJ)/tests/pkg+cprogram/cfile.c/cfile.o &: tests/pkg/cfile.c $(PACKAGE_DEP_missing) $(PACKAGE_DEP_ab-sample-pkg)
	$(hide) $(ECHO) CC tests/pkg+cprogram/cfile.c
	$(hide) mkdir -p $(OBJ)/tests/pkg+cprogram/cfile.c
	$(hide) $(CC) -c -o $(OBJ)/tests/pkg+cprogram/cfile.c/cfile.o tests/pkg/cfile.c $(CFLAGS) $(PACKAGE_CFLAGS_missing) $(PACKAGE_CFLAGS_ab-sample-pkg)
